...
public:
    ...
    bool _InitCaptureDevice(const char* uuid)
    {
        ...
        {
            /* use UUID */
            if (S_OK != GetCaptureDeviceWithUID(&_SourceFilter,uuid))
            {
                /* if that fails try the default */
                if (S_OK == GetCaptureDevice(&_SourceFilter))
                    return true;
            }
        } else
        {
        ...
        OnError("can't initialize capture device");
